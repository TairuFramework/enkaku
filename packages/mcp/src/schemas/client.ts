...
export function createClientMessage(protocol?: ProtocolDefinition): Schema {
  if (protocol?.tools == null) {
    return clientMessage
  }

  // Validate params for supported tool calls
  const callToolParams = Object.entries(protocol.tools).map(([name, tool]) => {
    return {
      type: 'object',
      required: ['name'],
      properties: {
